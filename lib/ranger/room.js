...
exports.Room = function (connection, attrs) {
  ...
  /**
   * Gets the guest access token (or null if guest access is disabled).
   * @param {function(string)} callback The callback to call with the token.
   */
  this.guestToken = function (callback) {
    get(null, function (data) {
      if (data.room.open_to_guests) {
        callback(data.room.active_token_value);
      } else {
        callback(null);
      }
    });
